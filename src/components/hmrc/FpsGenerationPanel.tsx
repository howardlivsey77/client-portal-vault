...
interface FpsGenerationPanelProps {
  defaultTaxYear?: string;
  defaultTaxPeriod?: number;
}

export function FpsGenerationPanel({ defaultTaxYear, defaultTaxPeriod }: FpsGenerationPanelProps) {
  const financialYears = useMemo(() => generateFinancialYears(), []);
  const [taxYear, setTaxYear] = useState(defaultTaxYear || financialYears[1]);
  const [taxPeriod, setTaxPeriod] = useState<string>(String(defaultTaxPeriod || 1));
      ...
      <CardContent>
        {!result ? (
          <div className="space-y-4">
            ...
            <Button onClick={handleGenerate} disabled={isLoading} className="w-full max-w-md">
              {isLoading ? (
                <>
