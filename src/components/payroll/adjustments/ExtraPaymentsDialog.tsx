...
import { ExtraPaymentItem } from './types';
import { formatPounds } from '@/lib/formatters';

interface ExtraPaymentsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  employeeName: string;
  initialItems: ExtraPaymentItem[];
  onSave: (items: ExtraPaymentItem[]) => void;
}

export function ExtraPaymentsDialog({
  open,
  onOpenChange,
  employeeName,
  initialItems,
  onSave,
}: ExtraPaymentsDialogProps) {
  const [items, setItems] = useState<ExtraPaymentItem[]>(initialItems);
  ...
  function createEmptyItem(): ExtraPaymentItem {
    return {
  ...
  function updateItem(id: string, field: keyof ExtraPaymentItem, value: string | number) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader className="flex flex-row items-center justify-between">
          <DialogTitle>Extra Payments - {employeeName}</DialogTitle>
