...
export function DocumentGrid({
  ...
  return <div className="space-y-6">
      ...
      <DocumentGridHeader selectedFolderId={selectedFolderId} onNavigateBack={onNavigateBack} folderPath={folderPath} onAddDocument={onAddDocument} />
      
      {/* Subfolders section */}
      {selectedFolderId && <SubfolderList subfolders={subfolders} onFolderSelect={handleFolderSelect} />}
      
      {/* Document list */}
      {selectedFolderId && <>
          <h3 className="text-lg font-medium mb-3">Documents</h3>
          {documents.length > 0 ? <DocumentList documents={documents} /> : <EmptyFolder onAddDocument={onAddDocument} />}
        </>}
      
      {!selectedFolderId && <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
